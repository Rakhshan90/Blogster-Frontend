...
const UpdatePassword = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      //dispath the action
      dispatch(updateUserPasswordAction(values?.password));
    },
    validationSchema: formSchema,
  });

  //select profile and updatedPassword data from store
  const users = useSelector(state => state.users);
  const { userAuth, isPasswordUpdated, loading, appErr, serverErr } = users;
  //navigate
