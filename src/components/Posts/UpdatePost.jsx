...
export default function UpdatePost(props) {
  ...
  //fetch post details and then populate over the form
  useEffect(()=>{
    dispatch(fetchPostDetailsAction(id));
  }, [id, dispatch]);
  //select the post from store
  const posts = useSelector(state=>state.posts)
  const {postDetails} = posts;
  //select updated post from store
  const{postUpdated, loading, appErr, serverErr, isUpdated} = posts;
  // const{loading, appErr, serverErr} = postUpdated;
    ...
    <>
      <div className="min-h-screen bg-gray-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-md">
          ...
          {appErr || serverErr? (<h1 className="text-red-400 text-xl text-center">{appErr} {serverErr}</h1>) : null}
        </div>
